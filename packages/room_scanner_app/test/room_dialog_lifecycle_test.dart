...
void main() {
  ...
  for (final save in [false, true]) {
    testWidgets(
      'wall editor closes safely with focused field (save: $save)',
      (tester) async {
        ...
        final edit = find.byTooltip(l10n.editWallTitle(1)).first;
  ...
  for (final type in FeatureType.values) {
    testWidgets(
      'editing ${type.name} distance closes safely with field focused',
      (tester) async {
        ...
        final position = painter.transform(
          ARPoint(x: 1.5, y: 0, z: 0),
        );
        await tester.tapAt(tester.getTopLeft(canvas) + position);
        await tester.pumpAndSettle();
        final l10n = AppLocalizations.of(
          tester.element(find.byType(FloorPlanViewerScreen)),
        )!;
        await tester.tap(find.text(l10n.editOpeningDimensions));
        await tester.pumpAndSettle();
        final distance = find.byWidgetPredicate(
          (w) =>
              w is TextField &&
        ...
        expect(find.byType(AlertDialog), findsNothing);
        expect(
          provider.completedRooms.single.features.single.start.x,
          closeTo(2, 1e-8),
        );
      },
    );
  ...
  testWidgets(
    'room name and completion dialogs finish transitions before navigation',
    (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          locale: const Locale('en'),
      ...
      expect(
        await RecentRoomNamesService().load(),
        contains('Kitchen'),
      );
      expect(tester.takeException(), isNull);
    },
